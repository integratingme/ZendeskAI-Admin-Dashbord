...
export default function UserTesting() {
  ...
  // Set page title
  useEffect(() => {
    document.title = 'User Dashboard - Testing';
  }, []);
  ...
  const testSubscriptionLLM = async () => {
    ...
    try {
      ...
      const overallStatus = response.test_results?.overall_status?.toLowerCase();
      if (overallStatus === 'both_success' || overallStatus === 'excellent' || overallStatus?.includes('success')) {
        toast.success('Subscription test successful', 'LLM configuration test completed');
      } else if (overallStatus === 'main_only' || overallStatus === 'fallback_only') {
        const formattedStatus = formatOverallStatus(response.test_results?.overall_status || 'Unknown');
        toast.warning('Partially successful', `LLM test completed. Status: ${formattedStatus}`);
      } else {
        const formattedStatus = formatOverallStatus(response.test_results?.overall_status || 'Unknown');
        toast.error('Subscription test failed', `LLM test completed. Status: ${formattedStatus}`);
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <UserLayout activeSection="testing">
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium" style={{ color: theme === 'dark' ? 'var(--accent)' : '#000000' }}>
                  {formatOverallStatus(subscriptionTestResult.test_results?.overall_status || 'Unknown')}
                </span>
              </div>
              ...
              {subscriptionTestResult.test_results?.main_llm_test && (
                <div className="border rounded-lg p-3" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div className="text-sm" style={{ color: 'var(--foreground)', opacity: 0.8 }}>
                    Status: <span style={{ color: (subscriptionTestResult.test_results.main_llm_test.status === 'success') ? '#16a34a' : '#dc2626' }}>{subscriptionTestResult.test_results.main_llm_test.status}</span>
                  </div>
                  <div className="text-sm" style={{ color: 'var(--foreground)', opacity: 0.8 }}>
                  ...
                  {subscriptionTestResult.test_results.main_llm_test.error && (
                    <div className="mt-2 p-2 rounded text-xs" style={{ background: 'rgba(239, 68, 68, 0.1)' }}>
                      {getFriendlyError(subscriptionTestResult.test_results.main_llm_test.error)}
                    </div>
                  )}
                </div>
              )}
              ...
              {subscriptionTestResult.test_results?.fallback_llm_test && (
                <div className="border rounded-lg p-3" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div className="text-sm" style={{ color: 'var(--foreground)', opacity: 0.8 }}>
                    Status: <span style={{ color: (subscriptionTestResult.test_results.fallback_llm_test.status === 'success') ? '#16a34a' : '#dc2626' }}>{subscriptionTestResult.test_results.fallback_llm_test.status}</span>
                  </div>
                  <div className="text-sm" style={{ color: 'var(--foreground)', opacity: 0.8 }}>
                  ...
                  {subscriptionTestResult.test_results.fallback_llm_test.error && (
                    <div className="mt-2 p-2 rounded text-xs" style={{ background: 'rgba(239, 68, 68, 0.1)' }}>
                      {getFriendlyError(subscriptionTestResult.test_results.fallback_llm_test.error)}
                    </div>
                  )}
                </div>
              )}
