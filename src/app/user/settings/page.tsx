...
export default function UserSettings() {
  ...
  // Set page title
  useEffect(() => {
    document.title = 'User Dashboard - Settings';
  }, []);
  ...
  return (
    <UserLayout activeSection="settings">
...
function LLMConfigurationSection({ subscription, onConfigUpdate }: LLMConfigurationSectionProps) {
  ...
  const validateMandatoryFields = () => {
    const { main_llm, fallback_llm } = llmConfig;
    return (
      main_llm.model.trim() !== '' &&
      main_llm.api_key.trim() !== '' &&
      main_llm.input_price_per_million > 0 &&
      main_llm.output_price_per_million > 0 &&
      fallback_llm.model.trim() !== '' &&
      fallback_llm.api_key.trim() !== '' &&
      fallback_llm.input_price_per_million > 0 &&
      fallback_llm.output_price_per_million > 0
    );
  };
  ...
  const handleSave = async () => {
    ...
    if (!validateMandatoryFields()) {
      toast.error('Validation Error', 'Please fill in all mandatory fields (Model, API Key, Input Price, Output Price) for both Main and Fallback LLMs');
      return;
    }
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  color: 'white',
                  border: 'none',
                  opacity: (saving || !validateMandatoryFields()) ? 0.6 : 1
                }}
              >
